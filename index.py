...
def get_svg():
    ...
    if not image_url:
        image_data = B64_PLACEHOLDER_IMAGE
    else:
        image_data = fetch_and_cache_image(image_url)
...
def daylist():
    ...
    if not daylist_phrase:
        return jsonify({"error": "No daylist found"}), 404

    if request.path == "/daylist/dark":
        color_scheme = "dark"
    elif request.path == "/daylist/light":
        color_scheme = "light"
    else:
        color_scheme = request.args.get("scheme", "light")
